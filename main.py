...
from ChatSocketListener import ChatSocketListener
import socket
...
def run_client(port="7575"):
    GlobalVars.CHAT_SOCKET_LISTENER = ChatSocketListener("127.0.0.1", int(port))
    # socket.gethostbyname(socket.gethostname())

if __name__ == '__main__':
    GlobalVars.LOGGER.debug("Starting client")
    t = Thread(target=run_client, args=("7588",))
    t.start()
